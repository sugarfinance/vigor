...
void eosusd::create( name   issuer,
...
void eosusd::setsupply( name issuer, asset maximum_supply )
{
    auto sym = maximum_supply.symbol;
    eosio_assert( sym.is_valid(), "invalid symbol name" );
...
void eosusd::issue( name to, asset quantity, string memo )
{
...
void eosusd::retire( asset quantity, string memo )
{
    auto sym = quantity.symbol;
    eosio_assert( sym.is_valid(), "invalid symbol name" );
    eosio_assert( memo.size() <= 256, "memo has more than 256 bytes" );
...
void eosusd::transfer(name    from,
                      name    to,
                      asset   quantity,
                      string  memo )
{
    ...
    if (to == _self){
      ...
      eosio_assert(quantity.symbol == symbol("EOSUSD",4), "Debt asset type must be EOSUSD");
      update(from);
...
void eosusd::sub_balance( name owner, asset value ) {
...
void eosusd::add_balance( name owner, asset value, name ram_payer )
{
...
void eosusd::open( name owner, const symbol& symbol, name ram_payer )
{
...
void eosusd::close( name owner, const symbol& symbol )
{
   require_auth( owner );
   accounts acnts( _self, owner.value );
...
void eosusd::assetin(name    from,
  ...
  if (itr == _user.end()) {
    itr = _user.emplace(_self,  [&](auto& new_user) {
      new_user.usern = from;
      new_user.debt = asset(0,symbol("EOSUSD", 4));
    });
      ...
      action(
        ...
        _self, 
        name("open"),
        std::make_tuple(from, symbol("EOSUSD", 4), _self)
      ).send();
  }
...
void eosusd::assetout(name usern, asset assetout, string memo) {
  ...
  if (memo.c_str() == string("collateral")){
    ...
    for (std::vector<asset>::const_iterator it = user.collateral.begin() ; it < user.collateral.end(); ++it){
      ...
      if (it->symbol == assetout.symbol){
        ...
        eosio_assert(((user.valueofcol) - ((assetout.amount/std::pow(10.0,it->symbol.precision())) * fxrate[assetout.symbol])) >= 1.01*(user.debt.amount/std::pow(10.0,4)),
...
void eosusd::borrow(name usern, asset debt) {

// todo: make this vectorized for multicollateral
  require_auth(usern);
  update(usern);
  auto &user = _user.get(usern.value,"User not found");
  eosio_assert(debt.symbol == symbol("EOSUSD",4), "Debt asset type must be EOSUSD");
 ...
 // issue(usern, debt,std::string("EOSUSD issued to ") + usern.to_string());
    action(
        permission_level{_self, name("active")},
        _self, 
        name("issue"),
        std::make_tuple(usern, debt, std::string("EOSUSD issued to ") + usern.to_string())
    ).send();
    update(usern);
}


double eosusd::pricingmodel(double scale, double collateral, asset debt, double stdev, uint64_t creditscore) {
...
void eosusd::update(name usern){

  auto &user = _user.get(usern.value,"User not found");

  double valueofcol = 0.0;
    for (std::vector<asset>::const_iterator it = user.collateral.begin() ; it != user.collateral.end(); ++it){
      valueofcol += (it->amount)/std::pow(10.0,it->symbol.precision()) * fxrate[it->symbol];
    }
  ...
  double valueofins = 0.0;
    for (std::vector<asset>::const_iterator it = user.insurance.begin() ; it != user.insurance.end(); ++it){
      valueofins += (it->amount)/std::pow(10.0,it->symbol.precision()) * fxrate[it->symbol];
    }
...
extern "C" {
  [[noreturn]] void apply(uint64_t receiver, uint64_t code, uint64_t action) {
    if((code==name("eosio.token").value || 
      ...
      eosio::execute_action(name(receiver),name(code), &eosusd::assetin);
    }
    if (code == receiver) {
            switch (action) { 
                EOSIO_DISPATCH_HELPER(eosusd, (create)(assetout)(borrow)(issue)(transfer)(open)(close)(retire)(setsupply)) 
            }    
    }
        eosio_exit(0);
  }
