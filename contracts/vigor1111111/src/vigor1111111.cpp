#include <vigor1111111.hpp>

void vigor1111111::doupdate()
{
   //require_auth(_self);
...
void vigor1111111::create( name   issuer,
...
void vigor1111111::setsupply( name issuer, asset maximum_supply )
{
    auto sym = maximum_supply.symbol;
    eosio_assert( sym.is_valid(), "invalid symbol name" );
...
void vigor1111111::issue( name to, asset quantity, string memo )
{
...
void vigor1111111::retire( asset quantity, string memo )
{
    auto sym = quantity.symbol;
    eosio_assert( sym.is_valid(), "invalid symbol name" );
    eosio_assert( memo.size() <= 256, "memo has more than 256 bytes" );
...
void vigor1111111::transfer(name    from,
                      name    to,
                      asset   quantity,
                      string  memo )
{
...
void vigor1111111::sub_balance( name owner, asset value ) {
...
void vigor1111111::add_balance( name owner, asset value, name ram_payer )
{
...
void vigor1111111::open( name owner, const symbol& symbol, name ram_payer )
{
...
void vigor1111111::close( name owner, const symbol& symbol )
{
   require_auth( owner );
   accounts acnts( _self, owner.value );
...
void vigor1111111::assetin( name   from,
                         name to,
                         asset  assetin,
                         string memo ) {
  if ( from == _self )
...
void vigor1111111::assetout(name usern, asset assetout, string memo) 
{
  require_auth(usern);
    ...
    if ( memo.c_str() == string("insurance") ) {
      ...
      for ( auto it = user.collateral.begin(); it != user.collateral.end(); ++it )
        if ( it->symbol == assetout.symbol ) { //User collateral type found
          ...
          statstable stats(name("datapreprocx"),name(issuerfeed[assetout.symbol]).value);
...
void vigor1111111::stresscol(name usern) {
...
double vigor1111111::portVarianceCol(name usern)
{
  ...
  for ( auto i = user.collateral.begin(); i != user.collateral.end(); ++i ) {
    ...
    statstable stats(name("datapreprocx"),name(issuerfeed[i->symbol]).value);
    auto itr = stats.find(1);
  ...
  for (auto j = i + 1; j != user.collateral.end(); ++j ) {
    ...
    statstable statsj(name("datapreprocx"),name(issuerfeed[j->symbol]).value);
...
double vigor1111111::portVarianceIns()
{
  ...
  for ( auto i = gstats.insurance.begin(); i != gstats.insurance.end(); ++i ) {
    ...
    for (auto j = i + 1; j != gstats.insurance.end(); ++j ) {
      ...
      statstable stats(name("datapreprocx"),name(issuerfeed[j->symbol]).value);
      auto itr = stats.find(1);
      double jVvol = (double)itr->vol/volPrecision;
      double jW = (double)itr->price[0] / pricePrecision;
      jW *= j->amount / std::pow(10.0, j->symbol.precision());
...
void vigor1111111::pricing(name usern) {
...
double vigor1111111::stressinsx(name usern) { // same as stressins, but remove remove the specified user
...
double vigor1111111::riskx(name usern)
...
void vigor1111111::pcts(name usern, double RM) { // percent contribution to solvency
...
void vigor1111111::payfee(name usern) {
  auto &user = _user.get( usern.value, "User not found" );
  eosio_assert(_globals.exists(), "no global table exists yet");
  globalstats gstats = _globals.get();
  ...
  for ( auto it = user.collateral.begin(); it != user.collateral.end(); ++it )
  {
    if ( it->symbol ==  vig) 
    {
      ...
      statstable stats(name("datapreprocx"),name(issuerfeed[vig]).value);
      auto itr = stats.find(1);
      ...
      else if (amt > 0)
      {
        ...
        bool found = false;
        asset def_premium = asset( 0, symbol("VIG", 4) ); // VIG
        auto itr = gstats.collateral.begin();
        while(!found && itr++ != gstats.collateral.end())
        {
          found = itr->symbol == vig;
          
           _user.modify(user, _self, [&](auto& modified_user){
            if(!found)
            {
              modified_user.insurance.push_back(def_premium); // insert the vig asset insidde here
            }
            else
            {
...
void vigor1111111::update(name usern) 
{
  eosio_assert(_globals.exists(), "globals not found");
  auto &user = _user.get(usern.value, "User not found");
  globalstats gstats = _globals.get();
  ...
  for ( auto it = user.insurance.begin(); it != user.insurance.end(); ++it ) {
    statstable stats(name("datapreprocx"),name(issuerfeed[it->symbol]).value);
    auto itr = stats.find(1);
    valueofins += (it->amount) / std::pow(10.0, it->symbol.precision()) * 
                  ( (double)itr->price[0] / pricePrecision );
  }
  for ( auto it = user.collateral.begin(); it != user.collateral.end(); ++it ){
    statstable statsj(name("datapreprocx"),name(issuerfeed[it->symbol]).value);
    auto itr = statsj.find(1);
...
*/
void vigor1111111::bailout(name usern) 
{
  eosio_assert(_globals.exists(), "globals not found");
  auto &user = _user.get(usern.value, "User not found");
  globalstats gstats = _globals.get();
...
extern "C" {
  [[noreturn]] void apply(uint64_t receiver, uint64_t code, uint64_t action) {
    if((code==name("eosio.token").value ||
      ...
      eosio::execute_action(name(receiver),name(code), &vigor1111111::assetin);
    }
    if (code == receiver) {
      switch (action) { 
          EOSIO_DISPATCH_HELPER(vigor1111111, (create)(assetout)(issue)(transfer)(open)(close)(retire)(setsupply) )
      }    
    }
    eosio_exit(0);
  }
