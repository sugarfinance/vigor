#include <vigortester1.hpp>

void vigortester1::doupdate()
{
   //require_auth(_self);
...
void vigortester1::create( name   issuer,
...
void vigortester1::setsupply( name issuer, asset maximum_supply )
{
    auto sym = maximum_supply.symbol;
    eosio_assert( sym.is_valid(), "invalid symbol name" );
...
void vigortester1::issue( name to, asset quantity, string memo )
{
...
void vigortester1::retire( asset quantity, string memo )
{
    auto sym = quantity.symbol;
    eosio_assert( sym.is_valid(), "invalid symbol name" );
    eosio_assert( memo.size() <= 256, "memo has more than 256 bytes" );
...
void vigortester1::transfer(name    from,
                      name    to,
                      asset   quantity,
                      string  memo )
{
...
void vigortester1::sub_balance( name owner, asset value ) {
...
void vigortester1::add_balance( name owner, asset value, name ram_payer )
{
...
void vigortester1::open( name owner, const symbol& symbol, name ram_payer )
{
...
void vigortester1::close( name owner, const symbol& symbol )
{
   require_auth( owner );
   accounts acnts( _self, owner.value );
...
void vigortester1::assetout(name usern, asset assetout, string memo) 
{
  require_auth(usern);
...
void vigortester1::stresscol(name usern) {
...
double vigortester1::portVarianceCol(name usern)
{
...
double vigortester1::portVarianceIns()
{
...
void vigortester1::pricing(name usern) {
...
double vigortester1::stressinsx(name usern) { // same as stressins, but remove remove the specified user
...
double vigortester1::riskx(name usern)
...
void vigortester1::pcts(name usern, double RM) { // percent contribution to solvency
...
void vigortester1::payfee(name usern) {
  auto &user = _user.get( usern.value, "User not found" );
  eosio_assert(_globals.exists(), "no global table exists yet");
  globalstats gstats = _globals.get();
...
void vigortester1::update(name usern) 
{
  eosio_assert(_globals.exists(), "globals not found");
  auto &user = _user.get(usern.value, "User not found");
  globalstats gstats = _globals.get();
...
*/
void vigortester1::bailout(name usern) 
{
  eosio_assert(_globals.exists(), "globals not found");
  auto &user = _user.get(usern.value, "User not found");
  globalstats gstats = _globals.get();
...
extern "C" {
  [[noreturn]] void apply(uint64_t receiver, uint64_t code, uint64_t action) {
    if((code==name("eosio.token").value ||
        code==name("vigtokenz111").value ||
        code==name("dummytokensx").value) && action==name("transfer").value) {
      eosio::execute_action(name(receiver),name(code), &vigortester1::assetin);
    }
    if (code == receiver) {
      switch (action) { 
          EOSIO_DISPATCH_HELPER(vigortester1, (create)(assetout)(issue)(transfer)(open)(close)(retire)(setsupply) )
      }    
    }
    eosio_exit(0);
  }
