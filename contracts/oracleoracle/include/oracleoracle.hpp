...
CONTRACT oracleoracle : public eosio::contract {
 public:
  oracleoracle(name receiver, name code, datastream<const char*> ds) : eosio::contract(receiver, code, ds) {}
...
extern "C" {
    void apply(uint64_t receiver, uint64_t code, uint64_t action) {
        if(code==receiver)
        {
            switch(action)
            {
                EOSIO_DISPATCH_HELPER(oracleoracle, (write)(clear)(claim)(configure)(transfer))
            }
        }
        else if(code=="eosio.token"_n.value && action=="transfer"_n.value) {
            execute_action( name(receiver), name(code), &oracleoracle::transfer);
        }
    }
}
