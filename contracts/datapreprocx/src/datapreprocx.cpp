#include <datapreprocx.hpp>

//add to the list of pairs to process
ACTION datapreprocx::addpair(name newpair) {
...
uint64_t datapreprocx::get_last_price(name pair, uint64_t quoted_precision){
...
void datapreprocx::averageVol(name aname){
...
void datapreprocx::averageCor(name aname){
...
void datapreprocx::calcstats(const name pair, const uint64_t freq){
...
int64_t datapreprocx::corrCalc(std::deque<int64_t> X, std::deque<int64_t> Y, uint64_t n) { 
...
double datapreprocx::volCalc(std::deque<int64_t> returns, uint64_t n) {
...
void datapreprocx::store_last_price(const name pair, const uint64_t freq, const uint64_t lastprice){
...
extern "C" {
  void apply(uint64_t receiver, uint64_t code, uint64_t action) {
      if(code==receiver)
      {
          switch(action)
          {
              EOSIO_DISPATCH_HELPER(datapreprocx, (update)(addpair)(clear))
          }
      }
